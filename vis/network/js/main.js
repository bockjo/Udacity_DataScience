d3.json("data.json",function(d) {
	...
	function configSigmaElements(config) {
		...
		var farben = [];
			for (b in sigInst.clusters) farben.push(b)
		debugger;
		// GET color to cluster name mapping
		
			
		var point = d["nodes"];
		var attr=[];
		point.forEach(function(d){
			attr.push(d["attributes"])
		})
		var rgb = [];
		point.forEach(function(d){
			rgb.push(d["color"])
		...
		rgb.forEach(function(d){
			mapping.add(d+";"+attr[i]["topic"])
			i+=1
		})
		
		var mapping = Array.from(mapping);
		...
		var prop = [];
			b,x=1;
			for (b in sigInst.clusters) prop.push(' (' + ((sigInst.clusters[b].length)*100/point.length).toFixed(2) +"%"+ ')' );
		
		var cluster = [];
		farben.forEach(function(d){
			cluster.push(Dict[d])
		})
		
		var cluster_name = [];
			cluster.forEach(function (value, i) {
				cluster_name.push(value + prop[i])
			})
	
		
		var a = [],
			b,x=1;
			for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> Group ' + (x++) +' (' + ((sigInst.clusters[b].length)*100/point.length).toFixed(2) +"%"+ ')</a></div>');
		
		
		var data=[];
			farben.forEach(function (value, i) {
